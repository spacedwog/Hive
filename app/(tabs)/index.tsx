...
import PushNotification from 'react-native-notification';
      ...
      try {
        ...
        if (data.data.tentativasBloqueadas > data.data.regrasAplicadas) {
          try {
            ...
            const ipParaBloquear =
              data.data.ip ||
              `192.168.1.${Math.floor(Math.random() * 254 + 1)}`; // Pode substituir por lista de IPs potenciais
            ...
            PushNotification.localNotification({
              ...
              message: `IP bloqueado: ${ipParaBloquear}\nNova regra: ${destination} -> ${gateway}`,
            });
        ...
        if (data.data.tentativasBloqueadas === data.data.regrasAplicadas && !routeSaved) {
          try {
            ...
            PushNotification.localNotification({
              title: 'Rota Aplicada',
              message: `Rota salva: ${destination} -> ${gateway}`,
            });
              ...
              {totalModalPages > 1 && (
                <View style={{ flexDirection: 'row', justifyContent: 'center', marginTop: 8 }}>
                  <TouchableOpacity
                    ...
                    onPress={() => setModalPage((prev) => prev - 1)}
                    style={{ marginHorizontal: 8 }}
                  >
          ...
          <View style={styles.modalContainer}>
            <View style={styles.modalContent}>
              ...
              <Text style={[styles.description, { marginBottom: 12 }]}>
                Existem {firewallData?.tentativasBloqueadas ?? 0} tentativas bloqueadas e{' '}
                {firewallData?.regrasAplicadas ?? 0} regras aplicadas.
              </Text>
              <View style={{ flexDirection: 'row', justifyContent: 'space-around' }}>
                <TouchableOpacity
                  style={[styles.loginBtn, { paddingHorizontal: 16 }]}
                  onPress={() => {
