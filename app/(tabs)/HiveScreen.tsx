...
import {
  ...
  MAX_POINTS,
  NodeStatus,
} from "../../hive_brain/hive_prime/EspManager.ts";
...
export default function HiveScreen() {
  ...
  const fetchStatus = React.useCallback(async () => {
    try {
        ...
        servers.map(async (server) => {
          ...
          if (node.ultrassonico_m !== undefined && node.ultrassonico_m < 0.1) {
            ...
            try {
              ...
              console.error("Falha ao reportar anomalia para Vercel:", err);
            }
          } else {
            node.anomaly = {
              detected: false,
              message: "",
              current_value: node.ultrassonico_m ?? 0,
            };
          }
      ...
      // Atualiza histórico local de leituras
      setHistory((prev) => {
        const next = { ...prev };
        responses.forEach((s) => {
  ...
  const sendCommand = async (server: string, command: string, payload?: any) => {
    try {
      ...
      if (command === "ping" && res.data.analog_percent !== undefined) {
        setPingValues((prev) => ({
          ...prev,
          [server]: res.data.analog_percent,
        }));
      }
      fetchStatus();
    } catch (err) {
            ...
            <Callout>
              <View style={{ padding: 4 }}>
                ...
                {s.ultrassonico_m !== undefined && (
                  <Text>📏 Distância: {s.ultrassonico_m.toFixed(2)} m</Text>
                )}
                {s.anomaly?.detected && (
                  <Text style={{ color: "red", fontWeight: "bold" }}>
                    ⚠️ Anomalia: {s.anomaly.message} (valor atual:{" "}
                    {s.anomaly.current_value.toFixed(2)})
                  </Text>
                )}
              </View>
            </Callout>
            ...
            {onlineStatus.map((s, idx) => {
              ...
              return (
                <View key={idx} style={styles.nodeBox}>
                  ...
                  {s.anomaly?.detected && (
                    <Text
                      style={[styles.statusText, { color: "red", fontWeight: "bold" }]}
                    >
                      ⚠️ Anomalia: {s.anomaly.message} (valor atual:{" "}
                      {s.anomaly.current_value.toFixed(2)})
                    </Text>
                  ...
                  <View style={styles.buttonRow}>
                    <Button
                      ...
                      disabled={!s.server}
                      onPress={() =>
                        s.server && sendCommand(s.server, "activate")
                      }
                    />
                    <Button
                      title="Desativar"
                      disabled={!s.server}
                      onPress={() =>
                        s.server && sendCommand(s.server, "deactivate")
                      }
                    />
                    <Button
                      title="Ping"
                      disabled={!s.server}
                      onPress={() =>
                        s.server && sendCommand(s.server, "ping")
                      }
                    />
                  </View>
