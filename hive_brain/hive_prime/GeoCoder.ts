...
export class GeoCoder {
  private cache: { [key: string]: { latitude: number; longitude: number } } = {};

  async geocode(location: string, retries = 3): Promise<{ latitude: number; longitude: number }> {
    if (!location) {
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
      try {
        ...
        console.error(`Erro ao geocodificar (tentativa ${attempt}):`, err.message || err);
        if (attempt < retries) {
