      ...
      for (const k in obj) {
        if (Object.prototype.hasOwnProperty.call(obj, k)) {
          ...
          if (Array.isArray(value)) {
            value.forEach((v) => keyValueArray.push({ key: keyPath, value: v }));
...
export default async function handler(req, res) {
  ...
  try {
    if (method === "GET" && action === "blocked") return res.status(200).json(Firewall.getBlocked());
    if (method === "GET" && action === "info") return res.status(200).json(Firewall.getInfo());
    if (method === "POST" && action === "block") return res.status(200).json(Firewall.block(body?.ip));
    if (method === "POST" && action === "unblock") return res.status(200).json(Firewall.unblock(body?.ip));
    if (method === "POST" && action === "route") return res.status(200).json(Firewall.addRoute(body?.destination, body?.gateway));
    if (method === "POST" && action === "nat") return res.status(200).json(Firewall.addNAT(body?.internalIP, body?.externalIP));
    if (method === "POST" && action === "vpn") return res.status(200).json(Firewall.setVPN(body?.enable));
    if (method === "GET" && action === "connections") return res.status(200).json(await Firewall.getConnections());
